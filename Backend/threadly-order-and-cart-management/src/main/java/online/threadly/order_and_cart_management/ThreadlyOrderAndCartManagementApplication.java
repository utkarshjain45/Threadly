...
public class ThreadlyOrderAndCartManagementApplication {
	...
	private static void loadEnv() {
		...
		while (currentDir != null) {
			...
			File serviceEnv = new File(currentDir, "threadly-order-and-cart-management/.env");
			if (serviceEnv.exists() && serviceEnv.isFile()) {
				parseAndSetEnv(serviceEnv);
			}
			File backendServiceEnv = new File(currentDir, "Backend/threadly-order-and-cart-management/.env");
